...
import { 
  ...
  Utensils, Coffee, Hotel, ChevronDown, ChevronUp, History
} from 'lucide-react';
...
const AdminDashboard = () => {
  const [data, setData] = useState({
    clotures:     [],
    stats: { 
      caTotal: 0, depensesTotal: 0, soldeNet: 0, 
      tauxOccupation: 0, chambresDispos: 0, caParPoint: []
    },
    alertesStock: [],
    activites:    []
  });
  ...
  const [showAudited, setShowAudited]       = useState(false); // accordéon
  ...
  const loadLiveStatus = useCallback(async () => {
    try {
      ...
      const depenses = (resDecharges.data || []).reduce((sum, item) => sum + (item.montant || 0), 0);
      ...
      setData({
        ...
        stats: {
          ...
          caParPoint:     resVentes.data?.parPointDeVente || []
        },
  ...
  const handleAuditAction = async (id, status) => {
    ...
    try {
      await axios.patch(
        `${API_URL}/clotures/${id}/audit`,
        { statusAudit: status, noteEcart: ecartNote || (status === 'Valide' ? 'Validé par RAF' : 'Écart constaté') },
        getAuthConfig()
      );
      setData(prev => ({
        ...prev,
  ...
  const applyFilter = (list) =>
    filter === 'tous' ? list : list.filter(r => r.pointDeVente === filter);

  const enAttente = applyFilter(data.clotures.filter(r => !r.audite));
  const auditees  = applyFilter(data.clotures.filter(r =>  r.audite));

  // ── Couleur par PDV ────────────────────────────────────────────────────────
  const pdvColor = (pdv) => {
    if (!pdv) return 'bg-slate-500';
    const p = pdv.toLowerCase();
    if (p.includes('recep')) return 'bg-blue-600';
    if (p === 'bar')         return 'bg-purple-500';
    if (p === 'resto')       return 'bg-orange-500';
    return 'bg-slate-500';
  };
    ...
    <div className="p-4 md:p-8 bg-[#FDFBF9] min-h-screen text-slate-900 font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-[#386D7F] p-6 rounded-[2.5rem] text-white relative overflow-hidden shadow-xl shadow-[#386D7F]/20">
          ...
          <div className="mt-4 h-1.5 w-full bg-white/20 rounded-full overflow-hidden">
            <div className="h-full bg-white transition-all duration-700" style={{ width: `${data.stats.tauxOccupation}%` }}/>
          </div>
      ...
      <div className="mb-10">
        <h3 className="text-[10px] font-black uppercase text-slate-400 mb-4 tracking-[0.2em] flex items-center gap-2">
          <div className="w-2 h-2 bg-emerald-500 rounded-full animate-ping"/> Flux Direct par Caisse
        </h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="flex justify-between items-center">
            <h3 className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Audit des Clôtures</h3>
            <select
          ...
          <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
            <div className="px-6 py-4 bg-amber-50/60 border-b border-amber-100 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black uppercase text-amber-600 tracking-widest">En attente d'audit</span>
              </div>
              <span className={`text-[10px] font-black px-3 py-1 rounded-full ${enAttente.length > 0 ? 'bg-amber-500 text-white' : 'bg-slate-100 text-slate-400'}`}>
                {enAttente.length}
              </span>
            </div>
            ...
            {enAttente.length === 0 ? (
              <div className="py-14 text-center">
                ...
                <p className="text-[10px] font-black uppercase text-slate-300 tracking-widest">Tout est à jour</p>
                <p className="text-[9px] text-slate-300 mt-1">Aucune clôture en attente</p>
              </div>
            ) : (
              <table className="w-full text-left">
                <tbody className="divide-y divide-slate-50">
                  {enAttente.map(report => (
                    <ClotureRow
                      key={report._id}
                      report={report}
                      pdvColor={pdvColor}
          ...
          {/* ── DÉJÀ AUDITÉES — ACCORDÉON ──────────────────────────────────── */}
          <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
            {/* Bouton accordéon */}
            <button
              onClick={() => setShowAudited(v => !v)}
            ...
            >
              ...
              <div className="flex items-center gap-3">
                <span className="text-[10px] font-black bg-emerald-50 text-emerald-600 px-3 py-1 rounded-full">
                  {auditees.length} clôture{auditees.length > 1 ? 's' : ''}
                </span>
                {showAudited
            ...
            {showAudited && (
              <div className="border-t border-slate-50">
                {auditees.length === 0 ? (
                  <p className="py-8 text-center text-[9px] font-black uppercase text-slate-300">
                    Aucune clôture auditée
                  </p>
                ) : (
                  <table className="w-full text-left">
                    <tbody className="divide-y divide-slate-50">
          ...
          <div className="bg-white p-7 rounded-[2.5rem] border border-slate-100 shadow-sm border-t-4 border-t-red-500">
            ...
            {data.alertesStock.length === 0 ? (
              <div className="py-4 text-center opacity-20">
                ...
                <p className="text-[9px] font-black mt-2 uppercase">Stock OK</p>
              </div>
              ...
              <div key={i} className="flex justify-between items-center p-4 bg-red-50/50 rounded-2xl mb-3 border border-red-100/50">
                <span className="text-[10px] font-black text-red-700 uppercase">{s.nom || s.name}</span>
                <span className="bg-red-600 text-white px-3 py-1 rounded-lg text-[10px] font-black shadow-sm">
                  {s.quantite ?? s.quantity}
                </span>
              </div>
            ))}
          </div>
      ...
      {selectedReport && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-slate-900/60 backdrop-blur-md p-4">
          <div className="bg-white rounded-[3.5rem] p-10 max-w-md w-full shadow-2xl relative">
            <button onClick={() => setSelectedReport(null)} className="absolute top-8 right-8 text-slate-300 hover:text-red-500">
              <X />
            </button>

            <div className="flex flex-col items-center text-center mb-8">
              <div className="p-5 bg-slate-50 rounded-full text-[#386D7F] mb-4">
                <ShieldAlert size={36}/>
              </div>
              <h2 className="text-2xl font-black uppercase italic tracking-tighter text-slate-800">
                Audit <span className="text-[#386D7F]">Versements</span>
              </h2>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">
                Caisse : {selectedReport.pointDeVente} · Caissier : {selectedReport.caissier}
              </p>
              {selectedReport.audite && (
                <div className="mt-3 flex items-center gap-2 bg-emerald-50 text-emerald-600 px-4 py-2 rounded-full">
                  <CheckCircle2 size={14} />
                  <span className="text-[9px] font-black uppercase">Déjà auditée — modification possible</span>
                </div>
              )}
            </div>
            ...
            <div className="space-y-5 mb-8">
              ...
              {/* Stats cash / mobile si disponibles */}
              {selectedReport.stats?.declare && (
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-slate-50 p-4 rounded-2xl text-center">
                    <p className="text-[8px] font-black uppercase text-slate-400 mb-1">💰 Espèces</p>
                    <p className="font-black text-slate-800">{(selectedReport.stats.declare.cash || 0).toLocaleString()} F</p>
                  </div>
                  <div className="bg-slate-50 p-4 rounded-2xl text-center">
                    <p className="text-[8px] font-black uppercase text-slate-400 mb-1">📱 Mobile</p>
                    <p className="font-black text-slate-800">{(selectedReport.stats.declare.mobile || 0).toLocaleString()} F</p>
                  </div>
                </div>
              )}
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  className="w-full p-6 bg-slate-50 rounded-[2rem] text-xs font-bold outline-none ring-2 ring-transparent focus:ring-[#386D7F]/10 h-28 resize-none transition-all"
                  value={ecartNote}
...
// ── Composant ligne clôture ────────────────────────────────────────────────────
const ClotureRow = ({ report, pdvColor, onSelect, isAudited }) => (
  <tr className={`hover:bg-slate-50/80 transition-colors group ${isAudited ? 'opacity-60' : ''}`}>
    ...
    <td className="p-5">
      <span className={`text-[9px] font-black uppercase px-3 py-1.5 rounded-full text-white ${pdvColor(report.pointDeVente)}`}>
        {report.pointDeVente}
      </span>
    </td>
    <td className="p-5">
      {isAudited ? (
        <div className="flex items-center gap-1.5 text-emerald-500 font-black text-[9px] uppercase">
          ...
          <span>{report.statusAudit || 'Audité'}</span>
        </div>
      ) : (
        <div className="flex items-center gap-1.5 text-amber-500 font-black text-[9px] uppercase animate-pulse">
          <Clock size={13} />
          En attente
        </div>
      )}
    </td>
    <td className="p-5 text-right font-black italic text-slate-800 text-sm">
      {(report.totalVentes || report.stats?.declare?.total || 0).toLocaleString()}
      <span className="text-[10px] opacity-30 not-italic ml-1">F</span>
    </td>
    <td className="p-5 pr-6 text-right">
      <button
        onClick={onSelect}
        className="p-2.5 bg-slate-50 text-slate-400 rounded-xl group-hover:bg-[#386D7F] group-hover:text-white transition-all transform group-hover:translate-x-1"
      >
  ...
  <div className="bg-white p-7 rounded-[2.5rem] border border-slate-100 shadow-sm relative group overflow-hidden hover:border-[#386D7F]/20 transition-all">
    ...
    <div className="relative z-10">
      ...
      <h2 className={`text-2xl font-black italic ${color}`}>
        {value?.toLocaleString()} <span className="text-[10px] font-bold opacity-30 not-italic">F</span>
      </h2>
    </div>
  </div>
);
